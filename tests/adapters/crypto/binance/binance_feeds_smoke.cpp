...
int main() {
  ...
  trade_spec.market = md::feed::MarketSegment::Spot;
  trade_spec.symbols = {"BTCUSDT"};
  trade_spec.kind = md::feed::FeedKind::Trade;
  ...
  assert(trade.spec.venue == md::feed::VenueId::Binance);
  assert(trade.feed_id == md::feed::stable_id32(
                              binance::make_feed_id_seed(
                                  trade.spec.market, trade.provider_feed_key)));

  binance::BinanceFeedSpec agg_trade_spec = trade_spec;
  agg_trade_spec.kind = md::feed::FeedKind::AggregateTrade;
  ...
  depth_spec.market = md::feed::MarketSegment::Spot;
  depth_spec.symbols = {"ETHUSDT"};
  depth_spec.kind = md::feed::FeedKind::BookUpdate;
  depth_spec.speed = md::feed::FeedSpeed::Ms100;
  ...
  partial_depth_spec.market = md::feed::MarketSegment::Spot;
  partial_depth_spec.symbols = {"BNBUSDT"};
  partial_depth_spec.kind = md::feed::FeedKind::BookSnapshot;
  partial_depth_spec.depth = md::feed::FeedDepth::Top20;
  partial_depth_spec.speed = md::feed::FeedSpeed::Ms100;
  ...
  book_top_spec.market = md::feed::MarketSegment::Spot;
  book_top_spec.symbols = {"BTCUSDT"};
  book_top_spec.kind = md::feed::FeedKind::TopOfBook;
  ...
  const auto connection_envelope =
      md::feed::make_connection_envelope(multi_connection, 3, 456);
  assert(connection_envelope.connection_id == 7);
  assert(connection_envelope.feed_id == 0);
  assert((connection_envelope.flags &
          md::feed::message_flag(md::feed::MessageFlag::FeedKnownWithoutParsing)) ==
         0);

  const auto envelope = md::feed::make_feed_envelope(
      single_connection, single_connection.feeds[0], 9, 123);
  assert(envelope.payload_kind == md::feed::PayloadKind::RawProviderMessage);
  assert(envelope.payload_encoding == md::feed::PayloadEncoding::ProviderJson);

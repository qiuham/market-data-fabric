...
int main() {
  ...
  md::runtime::SpscRing<md::feed::RawProviderMessageFrame<256>, 16> frames{};
  md::feed::RawProviderMessageFrame<256> frame{};
  frame.envelope.payload_kind = md::feed::PayloadKind::RawProviderMessage;
  frame.envelope.payload_encoding = md::feed::PayloadEncoding::ProviderJson;
  assert(md::feed::assign_payload(frame, std::string_view{"hello"}));
  assert(frames.try_push(frame));

  md::feed::RawProviderMessageFrame<256> popped{};

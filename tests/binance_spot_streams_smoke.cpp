...
int main() {
  ...
  const auto trade =
      binance::make_spot_stream_name("BTCUSDT", binance::SpotStreamKind::Trade);
  assert(trade == "btcusdt@trade");

  const auto agg_trade = binance::make_spot_stream_name(
      "BTCUSDT", binance::SpotStreamKind::AggregateTrade);
  assert(agg_trade == "btcusdt@aggTrade");

  const auto depth = binance::make_spot_stream_name(
      "ETHUSDT", binance::SpotStreamKind::DiffDepth,
      binance::SpotDepthLevels::Twenty, binance::SpotDepthSpeed::Ms100);
  assert(depth == "ethusdt@depth@100ms");

  const auto partial_depth = binance::make_spot_stream_name(
      "BNBUSDT", binance::SpotStreamKind::PartialDepth,
      binance::SpotDepthLevels::Twenty, binance::SpotDepthSpeed::Ms100);
  assert(partial_depth == "bnbusdt@depth20@100ms");

  const auto subscription = binance::make_spot_subscription(
      "BTCUSDT", binance::SpotStreamKind::BookTicker, 42,
      binance::SpotEnvironment::MarketDataOnly);
  assert(subscription.stream_name == "btcusdt@bookTicker");
  assert(subscription.url ==
  ...
  const auto combined = binance::make_spot_combined_ws_url(
      binance::SpotEnvironment::Production, streams);
  assert(combined == "wss://stream.binance.com:9443/"
                     "stream?streams=btcusdt@trade/ethusdt@depth@100ms");

  const auto envelope = binance::make_spot_envelope(subscription, 9, 123);

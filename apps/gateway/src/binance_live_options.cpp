...
struct FeedKindCliEntry {
  ...
  md::feed::FeedKind kind;
  bool supports_normalize{};
};

inline constexpr auto kFeedKindCliEntries =
    std::to_array<FeedKindCliEntry>({
        {"trade", md::feed::FeedKind::Trade, true},
        {"aggTrade", md::feed::FeedKind::AggregateTrade, false},
        {"bookTicker", md::feed::FeedKind::TopOfBook, true},
        {"depth", md::feed::FeedKind::BookUpdate, false},
        {"depth20", md::feed::FeedKind::BookSnapshot, false},
    });

[[nodiscard]] bool parse_feed_kind(std::string_view text,
                                   md::feed::FeedKind &feed_kind) noexcept {

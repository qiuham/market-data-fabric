...
struct SpotRawSubscription {
    std::string stream_name{};
    std::string url{};
    std::uint32_t source_id{kBinanceSpotSourceId};
    std::uint32_t connection_id{};
    std::uint32_t stream_id{};
    md::core::RawProtocol protocol{md::core::RawProtocol::WebSocket};
    md::core::RawPayloadCodec payload_codec{md::core::RawPayloadCodec::JsonText};
};
...
inline std::string make_spot_stream_name(std::string_view symbol,
                                         SpotStreamKind kind,
...
inline std::string make_spot_ws_base_url(SpotEnvironment environment, bool combined) {
    switch (environment) {
    ...
    return combined ? "wss://stream.binance.com:9443/stream?streams=" : "wss://stream.binance.com:9443/ws/";
}

inline std::string make_spot_raw_ws_url(SpotEnvironment environment, std::string_view stream_name) {
    return make_spot_ws_base_url(environment, false) + std::string(stream_name);
}

inline std::string make_spot_combined_ws_url(SpotEnvironment environment,
                                             const std::vector<std::string>& stream_names) {
    if (stream_names.empty()) {
        throw std::invalid_argument("Binance combined stream list must not be empty");
    }
...
inline SpotRawSubscription make_spot_raw_subscription(std::string_view symbol,
                                                      SpotStreamKind kind,
                                                      std::uint32_t connection_id,
                                                      SpotEnvironment environment = SpotEnvironment::MarketDataOnly,
                                                      SpotDepthLevels depth_levels = SpotDepthLevels::Twenty,
                                                      SpotDepthSpeed depth_speed = SpotDepthSpeed::Default) {
    SpotRawSubscription subscription{};
    subscription.stream_name = make_spot_stream_name(symbol, kind, depth_levels, depth_speed);
    subscription.url = make_spot_raw_ws_url(environment, subscription.stream_name);
    subscription.connection_id = connection_id;
    subscription.stream_id = fnv1a32(subscription.stream_name);
    return subscription;
}

inline md::core::RawEnvelope make_spot_raw_envelope(const SpotRawSubscription& subscription,
                                                    std::uint64_t capture_seq,
                                                    std::uint64_t recv_ts_ns,
                                                    md::core::RawMessageKind message_kind =
                                                        md::core::RawMessageKind::Data) noexcept {
    ...
    envelope.payload_codec = subscription.payload_codec;
    envelope.compression = md::core::RawCompression::None;
    envelope.message_kind = message_kind;
    envelope.flags = md::core::raw_frame_flag(md::core::RawFrameFlag::ProviderTimestampUnknown) |
                     md::core::raw_frame_flag(md::core::RawFrameFlag::StreamKnownWithoutParsing);
    return envelope;
}

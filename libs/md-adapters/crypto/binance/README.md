...
- `BinanceMappingContext` 复用 `md-core` 的最小 `InstrumentMappingContext`，Binance 本地只负责默认 source、venue、精度和 symbol。
...
- 等多个 crypto adapter 出现相同需求后，再提公共 mapper helper；当前只共享最小 instrument mapping context。

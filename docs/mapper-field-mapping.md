...
供应商字段转换必须放在每个 provider 自己的目录，不放在 `service`。原因是不同供应商的字段含义、时间语义、盘口模型、序号规则、异常值处理和重连行为都不同。Lv3 字段转换同样遵循这个规则。
...
```text
include/marketdata/providers/{market}/{asset_class}/{provider}/
  include/marketdata/providers/{provider}/
    *_field_mapper.hpp
    *_mapper.hpp
  src/
    *_field_mapper.cpp
...
生产 provider 的字段转换应由 C++ mapper 实现，或者由 `tools/codegen` 在构建期从外部文档项目生成 C++ 代码。运行时链路应该是：

```text
外部报文 -> C++ decoder -> C++ field mapper -> trading-core event
```

不应该是：

```text
外部报文 -> 读取/查询/解释 YAML -> trading-core event
```
...
不建议做一个全局 `md-mappers` 目录集中放所有供应商转换逻辑。字段转换应该和具体 provider 的 provider 代码、测试、文档放在一起，便于维护和升级。

如果多个 provider 后续确实有可复用逻辑，应提取成普通 C++ helper，并通过测试证明不会把供应商语义混在一起；具体字段转换仍然保留在 provider mapper 内。

...
供应商字段转换必须放在每个 adapter 自己的目录，不放在 `service`。原因是不同供应商的字段含义、时间语义、盘口模型、序号规则、异常值处理和重连行为都不同。Lv3 字段转换同样遵循这个规则。
...
生产 adapter 的字段转换应由 C++ mapper 实现，或者由 `tools/codegen` 在构建期从外部文档项目生成 C++ 代码。运行时链路应该是：

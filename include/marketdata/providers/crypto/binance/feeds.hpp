...
#include "marketdata/providers/crypto/binance/endpoints.hpp"
...
inline BinanceResolvedFeed resolve_symbol_feed(const BinanceFeedSpec &spec,
  ...
  feed.spec.venue = md::feed::VenueId::Binance;
  ...
  feed.feed_id = md::feed::stable_id32(
...
inline std::vector<BinanceResolvedFeed> resolve_feeds(
    const BinanceFeedSpec &spec) {
  if (md::feed::is_all_symbols(spec.symbols)) {
  ...
  for (const auto &symbol : spec.symbols) {
    if (md::feed::is_all_symbol_token(symbol)) {
...
inline BinanceConnectionSpec make_connection_spec(
  ...
  connection.protocol = md::feed::TransportProtocol::WebSocket;
  connection.compression = md::feed::Compression::None;

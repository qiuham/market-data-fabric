...
struct FeedMessageStats {
  ...
  void observe(const md::feed::FeedMessageView &message) noexcept {
    if (messages == 0) {
...
inline bool dispatch_feed_message(const md::feed::FeedMessageView &message,
...
public:
  [[nodiscard]] bool add(md::feed::FeedMessageHandler handler,

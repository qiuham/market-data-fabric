...
struct MapperRuntimeStats {
  ...
  void observe_success(const md::feed::FeedMessageView &message,
                       std::uint64_t events = 1) noexcept {
    observe_input(message);
    output_events += events;
  }

  void observe_failure(const md::feed::FeedMessageView &message) noexcept {
    observe_input(message);
    ++failures;
  }

private:
  void observe_input(const md::feed::FeedMessageView &message) noexcept {
    if (input_messages == 0) {
